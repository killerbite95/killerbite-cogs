...
class AdvancedCaptcha(commands.Cog):
    """Cog avanzado de Captcha con verificación automática al unirse al servidor y configuración personalizada."""
    ...
    def __init__(self, bot):
        ...
        default_guild = {
            "captcha_channel": None,          # ID del canal donde se enviará el captcha
            ...
            "max_attempts": 5,                # Número máximo de intentos para el captcha (por defecto 5)
        ...
        # Estructura en memoria para manejar tareas de captcha en curso: {user_id: asyncio.Task}
        self.captcha_tasks = {}
    ...
    async def on_member_join(self, member: discord.Member):
        """
        Al unirse un miembro, si no está en la lista de bypass y se ha configurado el canal,
        se inicia el proceso de captcha.
        """
        guild = member.guild
        ...
        # Inicia el proceso de captcha para este miembro
    ...
    async def start_captcha_process(self, member: discord.Member, channel: discord.TextChannel):
        """
        Inicia la secuencia de captcha:
          - Envía un embed personalizado y el mensaje con el captcha.
          - Espera hasta que el usuario responda correctamente o agote intentos/tiempo.
          - En caso de fallar, envía un DM con la invitación (si existe) y expulsa al usuario.
        """
        guild = member.guild
        guild_config = await self.config.guild(guild).all()
    ...
    async def fail_captcha(self, member: discord.Member, invite_link: Optional[str]):
        """Envía DM con invitación (si se configuró) y expulsa al miembro."""
        if invite_link:
            try:
                await member.send(
    ...
    async def safe_delete(self, message: discord.Message):
        """Intenta borrar un mensaje sin generar errores si no se tienen permisos."""
        try:
    ...
    async def setcaptchacolor(self, ctx, color: str):
        ...
        Formatos válidos: #RRGGBB o nombres como 'blue', 'red', etc.
    ...
    async def setupcaptcha(self, ctx):
        """
        Envía un mensaje con la secuencia de comandos que debes usar para configurar el captcha,
