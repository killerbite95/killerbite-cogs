...
class DashboardIntegration:
    ...
    async def rpc_servers_page(self, guild_id: int, **kwargs) -> typing.Dict[str, typing.Any]:
...
<div class="trini-tp-settings">
  ...
  <p class="trini-tp-subtitle">
    {{ online_count }}/{{ total_servers }} online &bull; {{ total_players }} jugadores
    <span class="badge bg-gradient-secondary ms-2">{{ timezone }} &bull; {{ refresh }}s</span>
  </p>

  {% if servers|length == 0 %}
    <div class="trini-tp-empty">
      <i class="fa fa-gamepad fa-3x"></i>
      <p>No hay servidores configurados en este servidor.</p>
    </div>
  {% else %}
